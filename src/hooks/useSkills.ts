...
export function useSkillsConversation(conversationId: string | null) {
  return useQuery<SkillDto[]>({
    queryKey: conversationId ? qkSkillsConv(conversationId) : ["skills", "conv", "none"],
...
export function useSetSkillConversationEnabled(conversationId: string) {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (args: { skillName: string; source: SkillSource; enabled: boolean }) =>
      ipc.setSkillConversationEnabled(conversationId, args.skillName, args.source, args.enabled),
    // 乐观更新：先改缓存，失败回滚
  ...
  return useMutation({
    ...
    onMutate: async ({ skillName, disabled }) => {
      ...
      qc.setQueryData<SkillDto[]>(QK_SKILLS_GLOBAL, (old) =>
        (old ?? []).map((s) => (s.name === skillName ? { ...s, globally_disabled: disabled } : s)),
      )
      return { prev }
    },
    onError: (_e, _vars, ctx) => {
